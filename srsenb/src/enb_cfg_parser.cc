...
int sib12_cell_parser::parse(libconfig::Setting& root)
{
    ...
    unsigned byte_code_line = std::count(
    ...
    // Set default Value of Message.
    std::fill_n(message, message_allocate, 'a');

    // Reutrn the iterator to the begining of the file.
    file.clear();
    file.seekg(0, std::ios::beg);

    if (file.is_open()) {
        int message_total = 0;
        int message_section_length = 0;    
        message[message_total++] = message_page;
        while (!file.eof()) {

            // Split to 3 section
            // [84n] ... [84n - 1]
            if ((message_total) % 83 == 0) {
                message[message_total++] = 0x52;
                message_section_length = 0;
            }
            else {
                unsigned char b;
                unsigned int c;
                ...
                message[message_total++] = b;
                message_section_length++;
            }
        }
